...
//    this.svgDice[6] += '</svg>';
  ...
  setDomNameAndStyle() {
    ...
    if (this.boardAppFlag) {
      $("#offtray1").css(this.getPosObj(11 * this.pointWidth, 50 * this.vh));
      $("#offtray2").css(this.getPosObj(11 * this.pointWidth, 0));
      ...
      $("#offtray2").css(this.getPosObj(12 * this.pointWidth, 0));
    }
    ...
    for (let i = 1; i < 21; i++) {
      const colfig = ((i>10) ? 1 : 0) * 2 + (i % 2); //0=under+even, 1=under+odd, 2=upper+even, 3=upper+odd
      const xh = '<div id="pt' + i + '" class="point ' + pointColorClass[colfig] + '"></div>';
      this.mainBoard.append(xh);
      this.point[i] = $('#pt' + i);
      const ey = (i > 10) ? 0 : this.mainBoardHeight - this.point[i].height();
    ...
    for (let i = 1; i < 21; i++) {
      ...
      this.mainBoard.append(xh);
      this.labels[i] = $('#lb'+i);
      const ey = (i > 10) ? this.upperlabelY : this.lowerlabelY;
  ...
  flipHorizOrientation() {
    this.flipHoriz();
    this.pointX[24] = (this.leftrightFlag) ? this.leftSideOff  - this.offtrayMargin
...
//    this.showBoard(this.xgidstr);
  ...
  showPosition(xg) {
    ...
    for (let player = 1; player <= 2; player++) {
      for (let i = piecePointer[player]; i < 13; i++) {
        const pt = (player == 1) ? 22 : 23;
        this.chequer[player][i].point = pt;
        this.chequer[player][i].stack = 13 - piecePointer[player];
      }
    }

    //駒をボードに並べる
    let ex, ey, ty, sf, bf;
    let ptStack = Array(24);
    ptStack.fill(0);
    for (let player = 1; player <= 2; player++) {
      for (let i = 0; i < 13; i++) {
        ...
        const which2 = (pt > 10 && this.topbottomFlag) || (pt <= 10 && !this.topbottomFlag);
        bf = false;

        if (pt == 22 || pt == 23) { //bear off
          bf = true;
          ex = this.pointX[22];
          sf = false;
          ey = (which == 1) ? this.offY[which] - (ptStack[pt] * this.boffHeight)
                            : this.offY[which] + (ptStack[pt] * this.boffHeight); //player==2
        } else if (pt == 0 || pt == 21) { //on the bar
          ex = this.pointX[pt];
          ...
          ey = (pt == 0) ? this.barY[which] + (ty * this.pieceHeight)
                         : this.barY[which] - (ty * this.pieceHeight); //pt==21
        } else { //in field
          ex = this.pointX[pt];
  ...
  animateChequer(xg, move, delay) {
    ...
    const hitflag = (topt == 21); // or (frpt < topt);
...
console.log("animateChequer ZINDEX", 50+num);
  ...
  findFromPointChequer(player, frpt, hitflag) {
    const frabs = (player == 1) ? frpt : 21 - frpt;
  ...
  countToPointChequer(player, toabs) {
    const ckerowner = (toabs == 0 || toabs == 21) ? BgUtil.getBdOppo(player) : player;
  ...
  calcAftPosition(player, topt) {
    const ckerowner = (topt == 21) ? BgUtil.getBdOppo(player) : player;

    let toabs;
    if (topt == 0)       { toabs = (player == 1) ? 22 : 23; } //bear off
    else if (topt == 21) { toabs = (player == 1) ? 0 : 21; } //to bar
    else                 { toabs = (player == 1) ? topt : 21 - topt; } //in field

    const which = (this.topbottomFlag) ? ckerowner : BgUtil.getBdOppo(ckerowner);
    const which2 = (toabs > 10 && this.topbottomFlag) || (toabs <= 10 && !this.topbottomFlag);
    const num = this.countToPointChequer(player, toabs);

    //動かす先の駒の位置を計算
    let ty, ey, ex, st, sf;
    if (toabs == 22 || toabs == 23) { //bear off
      ex = this.pointX[22];
      sf = false;
      ey = (which == 1) ? this.offY[which] - (num * this.boffHeight)
                        : this.offY[which] + (num * this.boffHeight); //player==2
    } else if (toabs == 0 || toabs == 21) { //on the bar
      ex = this.pointX[topt];
      ...
      ey = (toabs == 0) ? this.barY[which] + (ty * this.pieceHeight)
                        : this.barY[which] - (ty * this.pieceHeight); //topt==21
    } else { //in field
  ...
  bgBoardConfig() {
    ...
    const phr = this.mainBoardHeight / 11 / this.pieceWidth;
    ...
    if (this.boardAppFlag) {
      this.pointX = [5, 10,  9,  8,  7,  6,  4,  3,  2,  1,  0,
    ...
    this.dice10X = this.pointX[2];
    this.dice11X = this.pointX[4];
    this.dice20X = this.pointX[7];
    this.dice21X = this.pointX[9];
    ...
    if (this.boardAppFlag) {
      this.pointX[22] += offtrayMarginNum;
    } else {
      this.pointX[22] = (this.leftrightFlag) ? this.leftSideOff  - this.offtrayMargin
                                             : this.rightSideOff + this.offtrayMargin;
    }
  }
  ...
  getBarPos(player) {
    return this.getPosObj(this.pointX[21], this.barY[player]);
  }

  getDragEndPoint(pos, player) {
    const pos2ptz = [11,12,13,14,15,21,16,17,18,19,20,0,10,9,8,7,6,21,5,4,3,2,1,0];
