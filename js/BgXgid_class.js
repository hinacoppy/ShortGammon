// BgXgid_class.js
// ShortGammon Version (20pt Board and 13chequer)
'use strict';
...
class Xgid {
  constructor(xgid) {
    ...
    this._ptno = new Array(22);
    this._ptcol = new Array(22);
    this._pip = [0, 0];
    this._boff = [0, 0];
    this._gamesc = [[], []];
    ...
    this._turnpos = ((p) => (this._turn == 1) ? p : 21 - p);
  ...
  _parse_position(pt) {
    ...
    for (let i=0; i<=21; i++) {
      ...
      if (asc == "-".charCodeAt(0)) {
        ...
        if (i< 6) { this._bgarea[1] += this._ptno[i]; }
        if (i<16) { this._gmarea[1] += this._ptno[i]; }
        this._pip[1] += this._ptno[i] * (21 - i); // ピップ数を計算
      } else if (asc >= "A".charCodeAt(0) && asc <= "Z".charCodeAt(0)) {
        this._ptno[i] = asc - "A".charCodeAt(0) + 1;
        this._ptcol[i] = 1;
        this._boff[0] -= this._ptno[i];
        if (i>15) { this._bgarea[0] += this._ptno[i]; }
        if (i> 5) { this._gmarea[0] += this._ptno[i]; }
...
//public and private(helper) functions
  ...
  moveChequer(pos, move, turn) {
    ...
    for (let mv of BgUtil.cleanupMoveStr(move, this._xgid)) {
      frto = mv.split("/");
      fr = parseInt(frto[0]); fpt = (turn == 1) ? fr : 21 - fr;
      to = parseInt(frto[1]); tpt = (turn == 1) ? to : 21 - to; bar = (turn == 1) ? 0 : 21;
      if (isNaN(fr)) { break; }
  ...
  isValid() {
    if (this._dice_ary[1] > 5 || this._dice_ary[1] < 0) { return false; }
    if (this._dice_ary[2] > 5 || this._dice_ary[2] < 0) { return false; }
